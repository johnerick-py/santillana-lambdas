...
kinesis_client = session.client(
    "kinesis", region_name="us-east-1"  # Substitua pela sua região
)
...
BASE_JSON = {
        ...
        "data": {
            "eventId": "8123a63e-286f-4458-9008-19d5507cfc26",
            "attributes": {
...
def publish_record(record_data, shard_number):
    ...
    try:
        ...
        response = kinesis_client.put_record(
            ...
            PartitionKey="userarticlenotification",  # Chave de partição fixa como solicitado
        ...
        return {
            ...
            "record": record_data,
            "target_shard": (
                SHARDS[shard_number] if shard_number < len(SHARDS) else "unknown"
            ),
        }
...
def worker(worker_id, total_messages, results, start_index):
    ...
    for i in range(start_index, start_index + total_messages):
        ...
        if (i - start_index + 1) % 10 == 0:
            with print_lock:
                print(
                    f"Worker {worker_id}: {i - start_index + 1}/{total_messages} mensagens enviadas"
                )
    ...
    for result in data["results"]:
        if result["success"] and "datepublish" in result["record"]:
            try:
                ...
                sequence_time = (
                    int(result["sequence_number"][:13]) / 1000
                )  # Primeiros 13 dígitos são timestamp em milissegundos
...
verbID:  (tabela alvo: lrs_events_initialized)
=======================================
"""
    )
