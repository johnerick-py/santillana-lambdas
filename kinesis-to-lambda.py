...
def extract_verb_id_from_kinesis_data(data):
    """
    Extrai o verbID do evento Kinesis, considerando o formato específico dos dados
    """
    # Verifica se os dados contêm um campo "verb" que contém "id"
    if isinstance(data, dict):
        # Tenta extrair de acordo com o formato visto nos logs
        if "verb" in data and isinstance(data["verb"], dict) and "id" in data["verb"]:
            return data["verb"]["id"]
        # Mantém compatibilidade com o formato original se existir
        elif "verbID" in data:
            return data["verbID"]
...
def lambda_handler(event, context):
    ...
    for i, record in enumerate(event['Records']):
        ...
        try:
            ...
            # Extrair o verbID do novo formato de evento
            verb_id = extract_verb_id_from_kinesis_data(json_data)
            ...
            print(f"JSON transformado: {json.dumps(transformed_json, indent=2)}")
